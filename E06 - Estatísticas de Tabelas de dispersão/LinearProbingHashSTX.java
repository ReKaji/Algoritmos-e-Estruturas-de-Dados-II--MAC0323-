...
public class LinearProbingHashSTX<Key, Value> {
    ...
    public double hitProbes(){
        ...
        for(int j=0;j<keys.length;j++){
            if (keys[j]!=null){
                ...
                real_pos=hash(i);
                conta=1;
                while (!i.equals(keys[real_pos])) {
                    if (real_pos<keys.length-1)real_pos++;
                    else real_pos=0;
                    conta++;
                }
                soma+= conta;
    ...
    public int[] clusters(){
        int[] clusters = new int[m];
        int tam_clust=0;
        int j=0;
        for (int i=0;i<m;i++){
            if (keys[i]==null && tam_clust!=0){
                clusters[j]=tam_clust;
                tam_clust=0;
                j++;
            }
            else if (keys[i]!=null){
                tam_clust++;
                if (i==m-1) {clusters[j]=tam_clust;j++;}
            }
        }

        int[]vet_saida=new int[j];
        for (int i=0;i<j;i++){
            vet_saida[i]=clusters[i];
    ...
    public int maxClusterSize(){
        ...
        for (int i=0; i<clusters.length;i++){
            if (clusters[i]>max)max=clusters[i];
        }
        return max;
    }
